...
static bool isExecutableAddr(uint32_t addr)
{
	...
	// /proc/self　が正しい場所を示していないので以下のように明示的に指定する。
...
{
	log("registerInlineHook is called");
	log("target_addr = %x", target_addr);
	log("new_addr = %x", new_addr);

	struct inlineHookItem *item;


	//Excutableの判定を省略するとうまく動いた。

	//if (!isExecutableAddr(target_addr))
	//{
	//	log("target addr is not excutable");
	//}
	//
	//if (!isExecutableAddr(new_addr))
	//{
	//	log("new addr is not excutable");
	//}

	/*if (!isExecutableAddr(target_addr) || !isExecutableAddr(new_addr)) {
		log("ERROR_NOT_EXECUTABLE");
